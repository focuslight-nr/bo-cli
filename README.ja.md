...
- **ローカルファイル**: Mac上の音楽フォルダ(デフォルト `~/Music`。パス直接入力のほか、
  localhostで開いている場合は「参照…」からネイティブのフォルダ選択ダイアログで指定可)をブラウズして
  mp3/m4a/flac/wav等をスピーカーへストリーム再生。Range対応配信なのでシークも効く。
  埋め込みカバーアートと総時間も表示(MozartがURIストリームでは報告しないため
  mutagen/afinfoで補完)。曲が終わると同フォルダの次のファイルを自動再生し、
  リピート(オフ/全曲/1曲)とシャッフル設定に従う — アルバム再生がそのまま動く。
  再生中はMacを起こしておく必要あり
...
- PC向けの2段レイアウト切替(ローカルファイルを右列に表示)。こちらも選択を記憶
